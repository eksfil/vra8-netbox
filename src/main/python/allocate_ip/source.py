...
def allocate_in_range(range_id, auth_credentials, resource, allocation, context, endpoint):
    ...
    logging.info(f"netboxObject configured as: {netbox_object}")
    ...
    logging.info(f"Requested range_id: {range_id} (type: {type(range_id)})")
    logging.info(f"NetBox lookup response: {response.status_code} - {response.text}")
    logging.info(f"Returned r['id']: {r.get('id')} (type: {type(r.get('id'))})")

    if str(r["id"]) != str(range_id):  # ensure we have the correct prefix
        raise Exception(f"Range {str(r['id'])} does not match given ipRangeId: {str(range_id)}")

    addresses = requests.get(f"{str(r['url'])}available-ips/?limit=5", headers=headers, verify=verify)
    logging.info(f"available-ips raw response: {addresses.status_code} - {addresses.text}")
